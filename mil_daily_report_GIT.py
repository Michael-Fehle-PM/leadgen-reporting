...
mil_daily_report.py
==================
Produces the MIL Daily Leads workbook — one tab per date in the range,
each tab showing:
...
Override via command line:
    python mil_daily_report.py --start 2026-05-01 --end 2026-05-10
...
QUERY_FILE      = os.path.join(SCRIPT_DIR, "query_mil_daily.sql")
...
def build_sheet(wb, report_date, day_data, first_sheet=False):
    ...
    ws["A1"] = f"MIL Daily Leads  –  {date_label}"
    ...
    ws["B2"].fill      = _fill(MID_BLUE)
    ws["B2"].alignment = _center()
    ws["B2"].border    = _border()
    for ci in [1, 5]:
        ws.cell(row=2, column=ci).fill = _fill(MID_BLUE)
    ...
    summary_headers = [
        (1,  "",              DARK_BLUE, WHITE, None),
        (2,  "Collected",     MID_BLUE,  WHITE, None),
        (3,  "Sent",          MID_BLUE,  WHITE, None),
        (4,  "Δ",             MID_BLUE,  WHITE, None),
        (5,  "",              MID_BLUE,  WHITE, None),
    ...
    def put_sum(ci, value, fmt=None, font_c="000000", bold=False, fill_c=LIGHT_BLUE, bdr=None):
    ...
    put_sum(1, date_label, font_c=DARK_BLUE, bold=True)
...
def _output_path(start_dt, end_dt):
    ...
    path = os.path.join(base_dir, f"MIL_Daily_Leads_{stamp}.xlsx")
    if not os.path.exists(path):
        return path
    counter = 1
    while True:
        path = os.path.join(base_dir, f"MIL_Daily_Leads_{stamp}_{counter}.xlsx")
        if not os.path.exists(path):
            return path
        counter += 1
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="MIL Daily Leads Report")
