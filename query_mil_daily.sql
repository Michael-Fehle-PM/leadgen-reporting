-- ============================================================
-- MIL DAILY LEADS QUERY
-- Database : LEADS
-- BU       : MIL  (LEAD_DATA.FSRC = 'default-flow')
--
...
--                   filtered where FSRC = 'default-flow', independent of partner
-- Leads Sent      : LEAD_FOR_PARTNER where SEND_FLAG=1, joined to
--                   LEAD_DATA where FSRC='default-flow', grouped by SEND_DATE
-- ============================================================
...
-- ── MIL partner list: active contract + at least 1 MIL send ──────────
mil_partners AS (
    ...
    WHERE lfp.SEND_FLAG = 1
      AND ld.FSRC       = 'default-flow'
),

-- ── Leads collected per day (UTC → ET, FSRC='default-flow', all leads) ─
collected_daily AS (
    SELECT
        TRUNC(ld.CREATED_DATE + (:utc_offset / 24)) AS report_date,
        COUNT(*)                                     AS leads_collected
    FROM LEADS.LEAD_DATA ld
    WHERE ld.FSRC = 'default-flow'
...
-- ── Leads sent per day per partner (FSRC='default-flow') ─────────────
sent_daily AS (
    SELECT
...
-- ── Daily total leads sent (across all MIL partners) ─────────────────
sent_daily_total AS (
    SELECT
        report_date,
